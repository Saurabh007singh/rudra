...
export const AdminOrders = () => {
  ...
  const { singleOrder, isSingleOrderLoading } = useSelector(
    (state) => state.orders
  );
  ...
  function handleUpdateStatus(event, orderId, formData) {
    ...
    if (searched) {
      dispatch(updateStatus({ orderId, formData })).then((data) => {
        if (data.payload.success) {
          dispatch(getSingleOrders(search.trim())).then(() => {
      ...
      dispatch(updateStatus({ orderId, formData })).then((data) => {
        if (data.payload.success) {
          dispatch(getAllOrders(user?.id)).then(() => {
            toast({ title: "status updated" });
          });
        }
      });
  ...
  function handleOrderDelete(item) {
    dispatch(deleteOrders(item)).then(() => {
      dispatch(getAllOrders(user?.id));
      setSearched(false);
      setSearch("");
    });
            ...
            <>
              <Card>
                <CardHeader className="flex flex-row justify-between">
                  ...
                  <div
                    onClick={() =>
                      handleDownloadFullExel(searched ? singleOrder : allOrders)
                    }
                ...
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {(searched ? singleOrder : allOrders)
                        .slice()
                        .map((item) => (
                          <TableRow key={item._id}>
                            ...
                            <TableCell>
                              {item.orderStatus === "received" ? (
                                <span className="bg-orange-500 p-1 rounded-xl text-white ">
                                  Received
                                </span>
                              ) : item.orderStatus === "inShipping" ? (
                                <span className="bg-blue-500 p-1 rounded-xl text-white ">
                                  In Shipping
                                </span>
                              ) : item.orderStatus === "delivered" ? (
                                <span className="bg-green-500 p-1 rounded-xl text-white ">
                                  Delivered
                                </span>
                              ) : item.orderStatus === "returned" ? (
                                <span className="bg-red-500 p-1 rounded-xl text-white ">
                                  Returned
                                </span>
                              ) : item.orderStatus === "inProcess" ? (
                                <span className="bg-violet-500 p-1 rounded-xl text-white ">
                                  inProcess
                                </span>
                              ) : item.orderStatus === "rejected" ? (
                                <span className="bg-red-900 p-1 rounded-xl text-white ">
                                  Rejected
                                </span>
                              ) : (
                                <>{item.orderStatus}</>
                                ...
                                <Dialog>
                                  ...
                                  <DialogContent>
                                    <div className=" font-bold flex flex-col items-center justify-center gap-3  ">
                                      {" "}
                                      <span className="text-red-500 motion-safe:animate-ping text-4xl mb-10">
                                        Warning!
                                      </span>
                                      <span className="">
